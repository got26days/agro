...
class BuhsController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
        ...
        if (!$request->has('_validate')) {
			...
			if($data['newarea']){
                ...
                foreach ( $splitcontents as $title )
                {
                    ...
                    $alldomains = Buh::where('slug', '=', $slug)->first();
                    if($alldomains){
            ...
            $del = Buh::where('slug', '=', null)->delete();
        ...
        if (!$request->ajax()) {
            ...
            if($data['newarea']){
                ...
                foreach ( $splitcontents as $title )
                {
                    ...
                    $newpages = Buh::where('slug', 'LIKE', '%'. $serslug .'%')->where('group', '=', $data['group'])->get();
